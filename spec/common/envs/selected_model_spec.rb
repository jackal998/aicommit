...
RSpec.describe Common::Envs::SelectedModel do
  ...
  describe "#fetch!" do
    before do
      allow($stdout).to receive(:puts).with("Please select a model: (1-2)")
      allow($stdout).to receive(:puts).with("1. gpt-3.5-turbo")
      allow($stdout).to receive(:puts).with("2. gpt-3-abc")
    ...
    it "prompts the user and returns the selected model" do
      ...
      expect($stdout).to receive(:puts).with("AI_COMMIT_SELECTED_MODEL saved to .env".green)
    ...
    it "exits the program if an invalid selection is made" do
      ...
      expect($stdout).to receive(:puts).with("Invalid selection, exiting program.")
  ...
  describe "#update!" do
    ...
    it "prompts the user for input, validates and saves selected model to the env file" do
      expect($stdout).to receive(:puts).with("Please select a model: (1-2)")
      expect($stdout).to receive(:puts).with("1. gpt-3.5-turbo")
      expect($stdout).to receive(:puts).with("2. gpt-3-abc")
      expect($stdout).to receive(:puts).with("AI_COMMIT_SELECTED_MODEL saved to .env".green)
      subject.update!
  ...
  describe "#get_user_input_display_messages" do
    ...
    it "uses the tap method to build the messages array" do
      ...
      expect(messages[1..].size).to eq(subject.send(:models_list).size)
    end
  end
  ...
  describe "#validate_model_id!" do
    context "when model exists in available models" do
      let(:valid_model_id) { "gpt-3.5-turbo" }
      let(:model_data) { [{"id" => valid_model_id}, {"id" => "gpt-4"}] }
    ...
    context "when model does not exist in available models" do
      it "displays error message and exits with status 1" do
        ...
        expect($stdout).to receive(:puts).with(/Error: Model 'nonexistent-model' is not available/).once
