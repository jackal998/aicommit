...
RSpec.describe Common::Envs::BaseBranch do
  ...
  describe "#fetch!" do
    ...
    it "prompts the user when the value is not set" do
      stub_const("ENV", {})
      expect($stdout).to receive(:puts).with("AI_COMMIT_BASE_BRANCH saved to .env".green)
  ...
  describe "#validate_branch_name!" do
    ...
    it "returns nil if branch doesn't exist" do
      ...
      expect($stdout).to receive(:puts).with(/Error: Branch '#{base_branch}' does not exist/).once
      expect($stdout).to receive(:puts).with(/Please enter a valid branch name/).once
