    ...
    class IgnoreFileChecker
      ...
      def ensure_env_in_ignore_file
        IGNORE_FILE_PATHS.each do |ignore_file_path|
          ...
          next if parsed_ignore_file_content(ignore_file_path, raw_ignore_file_content).any? { |line| line.start_with?(Common::Envs::Base::ENV_PATH) }
      ...
      def append_env_to_ignore_file(ignore_file_path, content)
        puts "Adding #{Common::Envs::Base::ENV_PATH} to #{ignore_file_path} file...".yellow

        case ignore_file_path
        when ".gitignore"
          content = "#{content.chomp}\n" unless content.end_with?("\n")
          content += "\n# Environment variables\n#{Common::Envs::Base::ENV_PATH}\n"
